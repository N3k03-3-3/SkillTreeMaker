...
- [ThemeData](#themedata)
- [ThemePresetLibrary](#themepresetlibrary)
...
#### `initialize(runtime_data: Dictionary, class_id: String = "default") -> void`

runtime データからステートを初期化する。すべてのノードを LOCKED にし、unlock_rule に従って CAN_UNLOCK を計算する。

- **引数**:
  - `runtime_data` - runtime.json の Dictionary
  - `class_id` - エントリポイントのクラス識別子（デフォルト: `"default"`）
- **備考**: `unlock_rule` が `"path_connected"` の場合、エントリノードから隣接するノードのみ CAN_UNLOCK になる
...
#### `deserialize(save_data: Dictionary, runtime_data: Dictionary, class_id: String = "default") -> void`
...
theme.json のデータ構造をオブジェクトとして保持し、デフォルト値の生成・辞書との相互変換・バリデーションを提供する。
...
|--------|----|----|------|
| `SCHEMA_VERSION` | `int` | `1` | スキーマバージョン |
| `DEFAULT_BG_TINT` | `String` | `"#1A1A2E"` | デフォルト背景ティント色 |
| `DEFAULT_NODE_SIZE` | `int` | `48` | デフォルトノードサイズ（px） |
| `DEFAULT_EDGE_WIDTH` | `int` | `3` | デフォルトエッジ幅（px） |
| `DEFAULT_PADDING` | `int` | `24` | デフォルトウィンドウパディング（px） |
| `DEFAULT_GLOW_COLOR_UNLOCK` | `String` | `"#88CCFF"` | can_unlock 状態のデフォルトグロー色 |
| `DEFAULT_GLOW_COLOR_UNLOCKED` | `String` | `"#FFD27D"` | unlocked 状態のデフォルトグロー色 |
| `DEFAULT_EDGE_COLOR_LOCKED` | `String` | `"#445066"` | ロック時のデフォルトエッジ色 |
| `DEFAULT_EDGE_COLOR_ACTIVE` | `String` | `"#88CCFF"` | アクティブ時のデフォルトエッジ色 |
| `DEFAULT_NODE_PRESET_KEY` | `String` | `"node_default"` | デフォルトノードプリセットキー |
| `DEFAULT_EDGE_PRESET_KEY` | `String` | `"edge_default"` | デフォルトエッジプリセットキー |

### プロパティ

| プロパティ | 型 | 説明 |
|-----------|----|----|
| `background` | `Dictionary` | 背景設定（texture, tint, parallax） |
| `window` | `Dictionary` | ウィンドウ設定（frame_9slice, padding） |
| `node_presets` | `Dictionary` | ノードプリセット一覧 |
| `edge_presets` | `Dictionary` | エッジプリセット一覧 |
| `effects` | `Dictionary` | エフェクト設定 |
| `fonts` | `Dictionary` | フォント設定 |

### スタティックメソッド

#### `create_default() -> ThemeData`

デフォルト値で初期化済みの ThemeData インスタンスを生成する。

- **戻り値**: デフォルト値が設定された ThemeData
...
- **引数**: `data` - JSON 辞書
- **備考**: `schema_version` が一致しない場合は `push_warning` を出力。存在しないキーはデフォルト値（空辞書）のまま残る

#### `to_dict() -> Dictionary`

テーマデータを辞書として書き出す。

- **戻り値**: `schema_version` を含む完全な JSON 辞書

#### `is_valid() -> bool`

テーマデータが有効かどうかを検証する。

- **戻り値**: 以下の条件をすべて満たす場合 `true`
  - `node_presets` に `DEFAULT_NODE_PRESET_KEY` が存在する
  - `edge_presets` に `DEFAULT_EDGE_PRESET_KEY` が存在する
  - `background` に `"tint"` が存在する
  - `window` に `"padding"` が存在する

#### `get_node_preset(preset_name: String) -> Dictionary`

指定名のノードプリセットを取得する。

- **引数**: `preset_name` - プリセット名
- **戻り値**: プリセットの Dictionary。存在しない場合は `DEFAULT_NODE_PRESET_KEY` にフォールバック。どちらも存在しない場合は空 Dictionary

#### `get_edge_preset(preset_name: String) -> Dictionary`

指定名のエッジプリセットを取得する。

- **引数**: `preset_name` - プリセット名
- **戻り値**: プリセットの Dictionary。存在しない場合は `DEFAULT_EDGE_PRESET_KEY` にフォールバック。どちらも存在しない場合は空 Dictionary
...
ビルトインテーマプリセットを提供するサービス。ThemeData に依存せず raw Dictionary を返す設計で、Pack 作成時に theme.json を生成するために使用する。

### 定数

| 定数名 | 型 | 値 | 説明 |
|--------|----|----|------|
| `PRESET_DEFAULT` | `String` | `"default"` | プリセット名: デフォルト |
| `PRESET_SPACE` | `String` | `"space"` | プリセット名: スペース |
| `PRESET_ARCANE` | `String` | `"arcane"` | プリセット名: アーケイン |
| `ALL_PRESET_NAMES` | `Array[String]` | `["default", "space", "arcane"]` | 全プリセット名の配列 |

### プリセット一覧

| プリセット名 | 背景色 | アンロック可能グロー | アンロック済みグロー | エッジ色 |
|-------------|--------|---------------------|---------------------|---------|
| `default` | `#1A1A2E`（深夜ブルー） | `#88CCFF` | `#FFD27D` | `#88CCFF` |
| `space` | `#0A0A1A`（漆黒） | `#00BFFF` | `#87CEEB` | `#00BFFF` |
| `arcane` | `#1A0A2E`（深紫） | `#BF00FF` | `#DA70D6` | `#BF00FF` |
...
- **戻り値**: `ALL_PRESET_NAMES` の複製

#### `create_preset(preset_name: String) -> Dictionary`

指定されたプリセット名に対応する theme.json 形式の Dictionary を生成する。

- **引数**: `preset_name` - プリセット名（`PRESET_DEFAULT` / `PRESET_SPACE` / `PRESET_ARCANE`）
- **戻り値**: theme.json 形式の Dictionary
- **備考**: 不明なプリセット名が指定された場合は `default` テーマを返す

#### `get_default_theme() -> Dictionary`

デフォルトテーマの Dictionary を返す。

- **戻り値**: default テーマの theme.json 形式 Dictionary

#### `get_space_theme() -> Dictionary`

スペーステーマの Dictionary を返す。

- **戻り値**: space テーマの theme.json 形式 Dictionary

#### `get_arcane_theme() -> Dictionary`

アーケインテーマの Dictionary を返す。

- **戻り値**: arcane テーマの theme.json 形式 Dictionary
...
```json
{
  ...
  "tree": {
    ...
    "entry_nodes": [
      {"class_id": "default", "node_id": "n_power_strike"},
      {"class_id": "warrior", "node_id": "n_power_strike"},
      {"class_id": "mage", "node_id": "n_fireball"}
    ],
    "unlock_rule": "path_connected",
    "layout": {
...
ノードの視覚的・機能的な種別を表すトップレベルフィールド。

| 値 | 説明 | デフォルトサイズ |
|----|------|----------------|
| `"minor"` | 通路ノード（小円） | 20px |
| `"notable"` | 名前付き重要ノード（大円） | 32px |
| `"keystone"` | ゲームルール変更ノード（菱形、二重枠） | 44px |
| `"socket"` | 外部挿入可能なソケット（角丸四角） | 24px |

定数は `SkillTreeModel.NODE_TYPE_MINOR` / `NODE_TYPE_NOTABLE` / `NODE_TYPE_KEYSTONE` / `NODE_TYPE_SOCKET` で参照可能。

#### entry_nodes

複数クラスのエントリポイントを定義する配列。`entry_node_id`（旧）の代替。

```json
"entry_nodes": [
  {"class_id": "default", "node_id": "n_001"},
  {"class_id": "warrior", "node_id": "n_010"}
]
```

`SkillTreeModel` のメソッド:
- `add_entry_node(class_id, node_id) -> bool`
- `remove_entry_node(class_id) -> bool`
- `get_entry_nodes() -> Array`
- `get_entry_node_for_class(class_id) -> String`

#### unlock_rule

アンロック判定のルールを切り替えるフィールド。

| 値 | 説明 |
|----|------|
| `"requires"` | 各ノードの `unlock.requires` 配列で個別指定（後方互換、デフォルト） |
| `"path_connected"` | エントリポイントからグラフ上で連続接続されたノードのみアンロック可（POE型） |
